<div class="page-header">
    <div class="col-md-4 text-xs-center text-md-left text-nowrap">
      <h1>
        ...
        Información general ventas del POS
      </h1>
    </div>
</div> 
            ...
            <div class="row">
              ...
              $totaldia = "SELECT sum(sub_pedido) FROM mov_r_pedidos where fecha_registro>='".date("Y-m-d")."' and tipo_documento='PPS'";
              $variable1 = $oGlobals->verPorConsultaPor($totaldia, 0); 
              $total_dia=$variable1[0];

              $totalmes = "SELECT sum(sub_pedido) FROM mov_r_pedidos where periodo='".date("m")."' and tipo_documento='PPS'";; 
              $variable2 = $oGlobals->verPorConsultaPor($totalmes, 0); 
              $total_mes=$variable2[0];

              $totalano = "SELECT sum(sub_pedido) FROM mov_r_pedidos where year='".date("Y")."' and tipo_documento='PPS'";
...
// traer fechas de la tabla mov_r 
  $fechas="select SUBSTRING(fecha_registro, 1, 10) as 'fecha'
  ,sum(sub_pedido) FROM mov_r_pedidos where tipo_documento='PPS' and periodo='".date("m")."' group by fecha order by fecha ASC";
...
$ventas="select SUBSTRING(fecha_registro, 1, 10) as 'fecha'
  ,sum(sub_pedido) FROM mov_r_pedidos where tipo_documento='PPS' and periodo='".date("m")."' group by fecha order by fecha ASC";
...
$unidades="select SUBSTRING(fecha_registro, 1, 10) as 'fecha'
  ,sum(cantidad_pedida) FROM mov_r_pedidos where tipo_documento='PPS' and periodo='".date("m")."' group by fecha order by fecha DESC";
...
<?=include 'tablas/tb-documentos-pos.php';?>
</div> 
      </div>
    </div>
</div>