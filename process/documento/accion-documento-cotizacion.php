...
session_start();
	...
	if(trim($_POST["text_cliente"]) != "" && trim($_POST["id_tipo_documento"]) != "" && trim($_POST["fecha_documento"]) != ""){	
		...
		if($tipo_d != 2){
			...
			if($documento != 2){
				...
				if($insert != 0) echo "<div class='exito'>El documento se ha creado con éxito</div> <script>setTimeout('self.location=\"../documentos/menu_detalle-cotizacion_$insert.html\"', 2000)</script>";
