...
session_start();
	...
	if(trim($_POST["documento"]) != "" && trim($_POST["nombre"]) != "" && trim($_POST["entrada"]) != "" && trim($_POST["valor_unitario"]) != ""){	
		...
		$cantidad  = $oGlobals->iFilter->process($_POST["entrada"]);
		$valor     = $oGlobals->iFilter->process($_POST["valor_unitario"]);

		$insert    = 0;
		$update    = 2;
		
		$documento = $oGlobals->verOpcionesPor("mov", " AND id = '$id_doc'", 0);
		$producto  = $oGlobals->verOpcionesPor("referencias", " AND codigo = '".trim($nombre[0])."'", 0);
		$mov_r     = $oGlobals->verOpcionesPor("mov_r", " AND documento = '".$documento["documento"]."' AND codigo = '".$producto["codigo"]."'", 0);
		...
		if($producto == 2) { echo "<div class='error'>El producto ingresado NO existe</div>"; return;}

		if($documento != 2 && $producto != 2){

			//echo $documento["id_empresa"];
			//echo $producto["id_empresa"];

			if($documento["id_empresa"] != $producto["id_empresa"]) { echo "<div class='error'>No puedes agregar productos de empresas diferentes</div>"; return;}

			$array['id_entrada'] 			= $documento["id"];
			$array['documento'] 			= $documento["documento"];
			$array['tipo_documento'] 		= $documento["tipo_documento"];
			$array['codigo'] 				= $producto["codigo"];
			$array['referencia'] 			= $producto["referencia"];
			$array['detalle'] 				= utf8_encode($producto["nombre"]);
			...
			$array['salida'] 				= "0";
			$array['entrada'] 				= $cantidad;
			$array['valor_sugerido'] 		= "0";
			$array['valor_unitario'] 		= $valor;
			$array['sub_total'] 			= $valor * $cantidad;
			...
			$array['dcto_finan'] 			= "0";
			$array['costo'] 				= $valor;
			$array['bodega_entrada'] 		= $documento["bodega_entrada"];
			...
			if($id == "") $insert = $oGlobals->insert_data_array($array, "mov_r");
			else 		  $update = $oGlobals->update_data_array($array, "mov_r", "id", $mov_r["id"]);
...
?>
				<script>
					html = '<tr id="tradfasasdf_<?= $producto["id"];?>">\
	                          ...
	                          <td align="center">\
	                          	<a href="#load_modulo" onClick="Funciones.cargar_modal_estructura(\'<?= $producto["id"]."_".$documento["id"];?>\', \'add_producto_a_compra\', \'load_modulo\', 2);" data-toggle="modal"><i class="btn btn-xs fa fa-edit"></i></a>\
