...
session_start();
	...
	if(trim($_POST['text_cliente']) != "" && trim($_POST['fecha_documento']) != "" 
		&& trim($_POST['id_tipo_documento']) != "" && trim($_POST['bodega_salida']) != "" && trim($_POST['id_empresa']) != ""){
		...
		$usuario  = $oGlobals->verOpcionesPor("terceros", " AND codigo = '".$cliente[0]."' ".$con_emp, 0);
		$tipo_doc = $oGlobals->verOpcionesPor("tipos_documentos", " AND id_tipo_origen = $id_tipo ".$con_emp, 0);
		
		if($usuario == 2) { echo "<div class='error'>El cliente ingresado NO existe, debes crear un cliente para hacer una factura</div>"; return;}
		...
		if($insert != 0)  	  echo "<div class='exito'>El documento se ha creado correctamente</div> <script>setTimeout('self.location=\"menu_detalle-factura_$insert.html\"', 2000)</script>";
