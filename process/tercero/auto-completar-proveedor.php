...
session_start();
		...
		if($tag != ""){
			...
			$con = " AND proveedor='1' AND (codigo LIKE ('%$tag%') OR  identificacion LIKE ('%$tag%') OR nombre LIKE ('%$tag%') OR apellido LIKE ('%$tag%')) $con_emp LIMIT 0, 20";
			
			$usuarios = $oGlobals->verOpcionesPor("terceros", $con, 1);
