...
session_start();
	...
	if($_POST["nombre"] != ""){	
		...
		if($tercero  == 2) {
			...
			if($insert != 0) echo '<div class="cloudAlertProd alert alert-success">'.$close.'<strong>Cliente creado correctamente</strong>'.$espacio.' <script>$("#codigo").val("'.$_POST["codigo"].'");</script></div> ';
			else			 echo '<div class="cloudAlertProd alert alert-danger">'.$close.' <strong>No se ha podido crear el cliente</strong>'.$espacio.'</div>';
		}
		else {
