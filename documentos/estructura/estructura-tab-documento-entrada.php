  ...
  <div class="table-responsive">
    <table class="table m-a-0 table-striped ">
      ...
      <tbody id="tr_body_ref_fac" class="font-size-14">
        ...
        <?php 
            $productos = $oGlobals->verOpcionesPor("mov_r", "AND documento = '".$documento["documento"]."'", 1);
            if($productos != 2){
        ...
        ?>
                <tr id="trttrr_<?= $producto["id"];?>">
                  ...
                  <td class="p-a-1" align="right"><?= utf8_encode($producto["entrada"]);?></td>
                  ...
                  <?php if($documento["cerrado"] == 0){?>
                        <td align="center" width="8%">
                          <a href="#load_modulo" onClick="Funciones.cargar_modal_estructura('<?= $producto["id"]."_".$id;?>', 'add_producto_a_devolucion', 'load_modulo', 2);" data-toggle="modal"><i class="btn btn-xs fa fa-edit"></i></a>
                          <i onclick="Funciones.eliminar_registro('<?= $producto["id"];?>','mov_r','rspdasds', 'trttrr_');" class="btn btn-xs fa fa-trash"></i>
                        </td>
                  <?php }?>
              </tr>
        <?php 
